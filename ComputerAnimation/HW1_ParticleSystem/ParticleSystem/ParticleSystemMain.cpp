...
void DrawPlane()
{
	Vector3d originNormal(0, 1, 0);
	Vector3d planeNormalUnit(g_MassSpringSystem.GetPlaneNormal().NormalizedCopy());
	Vector3d planeRefPoint(g_MassSpringSystem.GetPlaneRefPoint());
	
	double rotateAngle = acos(originNormal.DotProduct(planeNormalUnit)/originNormal.Length());
	Vector3d rotateAxis = originNormal.CrossProduct(planeNormalUnit).NormalizedCopy();
                    ...
                    for(int i = 0 ; i<60 ; i++)
                    {
                        for(int j = 0; j<60 ; j++)
                        {
                            glBegin( GL_QUADS );
								...
								Vector3d V1((-30.0 + (double)i), 0.0, (-30.0 + (double)j));
								V1.Rotate(rotateAngle, rotateAxis);
								V1 += planeRefPoint;
								glVertex3d(V1.x, V1.y, V1.z);

                                glNormal3d(0.0,1.0,0.0);
                                glTexCoord2d( (double)(i+1)/60.0 , (double)(j)/60.0);
								Vector3d V2((-30.0 + (double)(i + 1)), 0.0, (-30.0 + (double)j));
								V2.Rotate(rotateAngle, rotateAxis);
								V2 += planeRefPoint;
								glVertex3d(V2.x, V2.y, V2.z);

                                glNormal3d(0.0,1.0,0.0);
                                glTexCoord2d( (double)(i+1)/60.0 , (double)(j+1)/60.0);
								Vector3d V3((-30.0 + (double)(i + 1)), 0.0, (-30.0 + (double)(j + 1)));
								V3.Rotate(rotateAngle, rotateAxis);
								V3 += planeRefPoint;
								glVertex3d(V3.x, V3.y, V3.z);
                                
                                glNormal3d(0.0,1.0,0.0);
                                glTexCoord2d( (double)(i)/60.0 , (double)(j+1)/60.0);
								Vector3d V4((-30.0 + (double)i), 0.0, (-30.0 + (double)(j + 1)));
								V4.Rotate(rotateAngle, rotateAxis);
								V4 += planeRefPoint;
