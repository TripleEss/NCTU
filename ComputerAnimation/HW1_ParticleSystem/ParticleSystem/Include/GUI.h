...
int g_iSpinnerModelID = 1;
int g_iSpinnerModelIDlimit = 1;
...
GLUI_Checkbox *g_pCheckboxDrawModel;
...
GLUI_Spinner *g_pSpinnerModelID;
...
void GUIConfigInit()
{
    ...
    configFile.addOption("DrawModel",&bDrawModel);
    ...
    configFile.addOption("ModelNum",&g_iSpinnerModelIDlimit);
    ...
    g_iCheckboxDrawModel         = (bDrawModel)?1:0;
...
void GLUI_Control_CallBack(int a_iControl)
{
    ...
    else if(a_iControl == enControlID::DRAW_MODEL)
    {
        if(g_iCheckboxDrawModel == 1)
            g_MassSpringSystem.SetDrawModel(true);
        if(g_iCheckboxDrawModel == 0)
            g_MassSpringSystem.SetDrawModel(false);
    }
    ...
    else if(a_iControl == enControlID::MODELID)
    {
        g_MassSpringSystem.SetPauseSimulation(); 
        g_MassSpringSystem.SetModelID(g_iSpinnerModelID);
        g_MassSpringSystem.Reset();
        g_pButtonStart->enable();
        g_pButtonPause->disable();
    }
...
void GLUIInit()
{
	...
	g_pSpinnerModelID = new GLUI_Spinner(pObjectPanel, "Model ID", &g_iSpinnerModelID,
		enControlID::MODELID, GLUI_Control_CallBack);
	g_pSpinnerModelID->set_int_limits(1, g_iSpinnerModelIDlimit);
	g_pSpinnerModelID->set_speed(1);
	...
	g_pCheckboxDrawModel = new GLUI_Checkbox(pRenderPanel, "DrawModel", &g_iCheckboxDrawModel,
		enControlID::DRAW_MODEL, GLUI_Control_CallBack);
