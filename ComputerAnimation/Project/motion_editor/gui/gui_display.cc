...
Display::Display()
	...
	cartoon_filter_(new kinematics::CartoonFilter),
	cartoon_filtered_pose_sequence_(NULL),
...
void Display::Reset()
{
	...
	cartoon_filtered_pose_sequence_.clear();
}
...
void Display::SolveCartoonFilter()
{
	cartoon_filter_->set_time_step(
		param_->value<double>("time_step")
	);

	cartoon_filter_->set_amplitude(
		param_->value<double>("cartoon_filter.amplitude")
	);

	cartoon_filter_->set_sigma_window_size(
		param_->value<int>("cartoon_filter.sigma_window_size")
	);

	int bone_num = fk_solver_coll_->at(0)->skeleton()->bone_num();
	int frame_num = fk_solver_coll_->at(0)->motion()->frame_num();

	kinematics::PoseCollColl_t original_pose_sequence(frame_num, kinematics::PoseColl_t(bone_num));

	for (int f = 0; f < frame_num; ++f) {
		original_pose_sequence[f] = fk_solver_coll_->at(0)->ComputeSkeletonPose(f);
	}

	cartoon_filter_->Configure(original_pose_sequence);

	cartoon_filtered_pose_sequence_.clear();
	cartoon_filtered_pose_sequence_ = cartoon_filter_->ComputeFilteredMotion();

	for (int f = 0; f < frame_num; ++f) {
		cartoon_filtered_pose_sequence_[f][0].set_start_pos(cartoon_filtered_pose_sequence_[f][0].end_pos());
		for (int b = 1; b < bone_num; ++b) {
			int parent_idx = fk_solver_coll_->at(0)->skeleton()->bone_ptr(b)->parent->idx;
			cartoon_filtered_pose_sequence_[f][b].set_start_pos(cartoon_filtered_pose_sequence_[f][parent_idx].end_pos());
		}
	}


	LOGMSG << "Display::SolveCartoonFilter() Done";
}
...
void Display::ShowSkeleton(const int32_t skeleton_idx)
{
    ...
    for (int32_t bone_idx = 0; bone_idx < skeleton_coll_->at(skeleton_idx)->bone_num(); ++bone_idx)
    {
		...
		else if (!cartoon_filtered_pose_sequence_.empty())
		{
			Color_t modified_color = (*skeleton_color_)(skeleton_idx);
			modified_color.set_alpha(
				param_->value<float>("cartoon_filter.skeleton_light_alpha")
			);
			renderer_->set_color(modified_color);
		}
        else
		...
		if (!cartoon_filtered_pose_sequence_.empty())
		{
			renderer_->set_color(param_->value<Color_t>("cartoon_filter.skeleton_color"));
			const double bone_radius = param_->value<double>("bone_radius");

			for (auto pose = cartoon_filtered_pose_sequence_[frame_idx_].begin();
				pose != cartoon_filtered_pose_sequence_[frame_idx_].end();
				++pose)
			{
				renderer_->DrawCylinder(
					pose->start_pos(),
