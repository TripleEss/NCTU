                      ...
                      double *rnorm)
{
  ...
  for (cgit = 1; cgit <= cgitmax; cgit++) {
    ...
    //---------------------------------------------------------------------
    d = 0.0;
    //xxxxx #pragma omp parallel for reduction(+:d)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      d = d + p[j]*q[j];
    }
    ...
    // Now, obtain the norm of r: First, sum squares of r elements locally...
    //---------------------------------------------------------------------
    //xxxxx #pragma omp parallel for reduction(+:rho)
    for (j = 0; j < lastcol - firstcol + 1; j++) {
      rho = rho + r[j]*r[j];
    }
  ...
  //---------------------------------------------------------------------
  //xxxxx #pragma omp parallel reduction(+:sum) private(d)
  for (j = 0; j < lastcol-firstcol+1; j++) {
    d   = x[j] - r[j];
    sum = sum + d*d;
  }
                   ...
                   double shift)
{
  ...
  #pragma omp parallel
  #pragma omp for ordered private(j1,j2,nza,k)
  for (j = 0; j < nrows; j++) {
    if (j > 0) {
